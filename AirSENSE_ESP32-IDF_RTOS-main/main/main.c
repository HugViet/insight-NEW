...
TaskHandle_t insightTask_handle = NULL;
...
static void initialize_nvs(void)
{
    ...
    error = esp_insights_init(&config);
    if (error != ESP_OK) {
        ESP_LOGE(TAG, "Failed to initialize ESP Insights, err:0x%x", error);
    }
    ESP_ERROR_CHECK(error);

    /* Following code generates an example error and logs it */
    nvs_handle_t handle;
    error = nvs_open("unknown", NVS_READONLY, &handle);
    if (error != ESP_OK) {
        ESP_LOGE(TAG, "Test error: API nvs_open() failed, error:0x%x", error);
    }
...
void insight_task(void *parameters)
{
     while (true) {
...
// Initialize RTC module
    ...
    WIFI_initSTA();
#endif
xTaskCreate(insight_task, "ESP_insight", (1024 * 16), NULL, (UBaseType_t)15, &insightTask_handle);
}
